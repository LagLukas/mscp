...
from source.nbpso import Particle
...
class TestNBPSO(unittest.TestCase):
    ...
    def test_local_optima_update(self):
        ...
        instance[4][1] = 1
        set_cover = SetCover(instance)
        nbpso = NBPSO(6, set_cover, 50)
        other_pop = []
        sol_a = Solution(set_cover, np.ones(5))
        other_pop.append(Particle(sol_a, sol_a, np.ones(5), set_cover))
    ...
    def test_nbpso_approx(self):
        ...
        instance[4][1] = 1
        set_cover = SetCover(instance)
        nbpso = NBPSO(10, set_cover, 20)
        solution = nbpso.find_approximation()
